...
class MusixmatchApiService {
  private async makeRequest(endpoint: string, params: Record<string, string> = {}): Promise<any> {
    ...
    if (data.message.header.status_code !== 200) {
      throw new Error(`Musixmatch API error: ${data.message.header.status_code}`);
    }
  ...
  async searchSongs(query: string, pageSize: number = 10, page: number = 1): Promise<Song[]> {
    ...
    try {
      const data: MusixmatchSearchResponse = await this.makeRequest('/track.search', {
        q_track: query,
      ...
      return data.message.body.track_list.map(item => ({
        ...
        imageUrl: item.track.album_coverart_500x500 || item.track.album_coverart_350x350 || item.track.album_coverart_100x100,
  ...
  async getSongLyrics(trackId: string, song?: Song): Promise<string> {
    ...
    try {
      const data: MusixmatchLyricsResponse = await this.makeRequest('/track.lyrics.get', {
        track_id: trackId,
      });
  ...
  async searchByArtist(artistName: string, pageSize: number = 10): Promise<Song[]> {
